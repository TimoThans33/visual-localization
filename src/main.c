...
const char *get_content_type(const char* path) {
    ...
    if (last_dot) {
        ...
        if (strcmp(last_dot, ".gif") == 0) return "image/git";
        ...
        if (strcmp(last_dot, ".json") == 0) return "application/javascript";
...
/* store information on each connected clien */
struct client_info {
...
/* store the root of our linked list in global variable */
...
/* Closes the connection to a client and removes it from
...
/* Attempts to transfer a file to a connected client. Let's
now implement these functions one at a time */
// looks into /public
